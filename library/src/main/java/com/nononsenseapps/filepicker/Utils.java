...
public class Utils {
    ...
    private static final String CONTENT_PROVIDER_URL_START = "^([/\\\\]+)?content:[/\\\\]+";
    ...
    public static boolean isProviderUri(String uri) {
        return (Pattern.compile(CONTENT_PROVIDER_URL_START).matcher(uri).find());
    }

    public static Uri parseContentProviderUriFromFile(File file) {
        if (isProviderUri(file.getAbsolutePath()))
            return Uri.parse(file.getAbsolutePath().replaceAll(CONTENT_PROVIDER_URL_START, "content://"));
        else
